...
namespace gcore
{
	...
	*/
	...
	public:
		...
		**/
		explicit LogManager( const std::string& defaultLogName = DEFAULT_LOG );
		...
		**/
		void addLogListener( LogListener* logListener, const std::string& logName = "" );
		...
		**/
		void removeLogListener( LogListener* logListener, const std::string& logName = "" );
		...
		**/
		Log* createLog( const std::string& name, bool isNewFile = true );
		...
		**/
		void logMessage( const std::string& logname, const std::string& message );
	...
	private:
		...
		typedef std::tr1::unordered_map< std::string, Log* > LogIndex;
