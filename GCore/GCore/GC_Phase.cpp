...
namespace gcore
{
	...
	{
		GC_ASSERT( m_state == Phase::UNLOADED , std::string( "Load requested to phase " ) + m_name + " but it is not unloaded!" );
	...
	{
		GC_ASSERT( m_state == Phase::LOADED , std::string( "Activate requested to phase " ) + m_name + " but it is not loaded!" );
	...
	{
		GC_ASSERT( m_state == Phase::ACTIVE , std::string( "Terminate requested to phase " ) + m_name + " but it is not active!" );
	...
	{
		GC_ASSERT( m_state == Phase::LOADED , std::string( "Unload requested to phase " ) + m_name + " but it is not loaded!" );
	...
	void Phase::registerListener( Phase::Listener* phaseListener )
	{
		...
		GC_ASSERT( phaseListener != nullptr, std::string( "Tried to register a null listener in the phase ") + m_name  );
		GC_ASSERT( std::find( m_listenerList.begin(), m_listenerList.end(), phaseListener ) == m_listenerList.end(), std::string( "Tried to regiser an already registered listener in the phase " ) + m_name );
		///////
	...
	void Phase::unregisterListener( Phase::Listener* phaseListener )
	{
		...
		GC_ASSERT( phaseListener != nullptr, std::string( "Tried to unregister a null listener in the phase ") + m_name  );
		GC_ASSERT( std::find( m_listenerList.begin(), m_listenerList.end(), phaseListener ) != m_listenerList.end(), std::string( "Tried to unregiser a non registered listener in the phase " ) + m_name );
		///////
	...
	void Phase::changeState( Phase::State newState )
	{
		...
		*/
		...
		for( std::size_t i = 0; i < listenerCount; ++i )
		{
			...
			GC_ASSERT( phaseListener != nullptr, std::string( "Found a null listener in phase " ) + m_name );
