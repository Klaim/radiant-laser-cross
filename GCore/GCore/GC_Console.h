...
namespace gcore	//gcore context
{
	...
	/** Base class for console management system.
		...
		- uses std::string ( have to change to template character type )
		@par
		Using the console :
		@par
	...
	*/
	...
	public:
		...
		typedef boost::circular_buffer< std::string > EntryList;
		typedef boost::circular_buffer< std::string > TextList;
		typedef std::vector< std::string > ParameterList;
		typedef std::tr1::unordered_map< std::string , ConsoleCommandPtr > CommandIndex;
		...
		*/
		long injectKeys(const std::string& keys);
		...
		*/
		void entry(const std::string& entry_value);
		...
		*/
		...
		void execute( const std::string& entryalue )
		{
			entry( entryalue );
			executeEntry();
		}
		...
		*/
		ConsoleCommandPtr command(const std::string& name);
		...
		*/
		void defaultCommand(const std::string& name);
		...
		*/
		void commandCallPrefix(const std::string& commandCallPrefix);
 
		/** Current prefix used to identify a command call.
		*/
		const std::string& commandCallPrefix() const { return m_commandCallPrefix; }
	...
	protected:
		...
		/** Parse the string as a parameter list (separated with " " by default) and fill a vector with the parameters.
			...
			@param paramString std::string with all the parameters.
			@param paramList Vector that will receive the result of the parsing : each parameter found.
			@param paramSeparators std::string that will be used to find the different parameters.
		*/
		void parseParameters(const std::string& paramString, std::vector< std::string >& paramList,const std::string& paramSeparators= " ");
	...
	private:
		...
		std::string m_commandCallPrefix;
		...
		std::string extractCommandCall( const std::string& entry ) const;
		...
		void insertEntryText( const std::string& text );
