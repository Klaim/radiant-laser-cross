...
namespace gcore
{
	...
	*/
	...
	void TaskManager::registerTask( Task& task )
	{
		...
		GC_ASSERT_RETURN( task.state() == TS_UNREGISTERED && task.taskManager() == nullptr, "Tried to register an already registered task! Task : " << task.name() );
		GC_ASSERT_RETURN( std::find( m_registeredTasksList.begin(), m_registeredTasksList.end(), &task) == m_registeredTasksList.end() , "Tried to activate task already registered in this TaskManager! Task : " << task.name() );
		...
		if( ! task.name().empty() )
		{
			...
			GC_ASSERT_RETURN( m_namedTasksIndex.find( task.name() ) == m_namedTasksIndex.end() , "Tried to register a task with an already registered name! Task : " << task.name() );
	...
	void TaskManager::unregisterTask( Task& task )
	{
		...
		GC_ASSERT_RETURN( task.m_state != TS_UNREGISTERED && task.m_taskManager == this , "Tried to unregister an non registered task! Task : " << task.name() );
	...
	void TaskManager::activateTask( Task& task )
	{
		...
		GC_ASSERT_RETURN( task.m_taskManager == this , "Tried to activate task not registered in this TaskManager! Task : " << task.name()  );
		
		// task must be registered but not active nor paused
		GC_ASSERT_RETURN( task.m_state == TS_REGISTERED , "Tried to activate task already registered or paused! Task : " << task.name() );
		
		GC_ASSERT_RETURN( std::find( m_registeredTasksList.begin(), m_registeredTasksList.end(), &task) != m_registeredTasksList.end() , "Tried to activate task not registered in this TaskManager! Task : " << task.name() );
	...
	{
		// task not null
		GC_ASSERT_RETURN( task.m_state == TS_ACTIVE , "Tried to deactivate an unactive task!" );
		GC_ASSERT_RETURN( task.m_taskManager == this , "Tried to deactivate a task not registered in this task manager!" );
	...
	void TaskManager::terminateTask( Task& task )
	{
		...
		GC_ASSERT_RETURN( task.m_state != TS_UNREGISTERED , "Tried to terminate a not registered task! Task : "  << task.name() );
		
		// task registered here
		GC_ASSERT_RETURN( task.m_taskManager == this , "Tried to terminate a task not registered in this TaskManager! Task : " << task.name() );
		GC_ASSERT_RETURN( std::find( m_registeredTasksList.begin(), m_registeredTasksList.end(), &task) != m_registeredTasksList.end() , "Tried to activate task not registered in this TaskManager! Task : " << task.name() );
		/////////////////////////////
	...
	void TaskManager::pauseTask( Task& task )
	{
		...
		GC_ASSERT_RETURN( task.m_state == TS_ACTIVE , "Tried to pause a non active task! Task : " << task.name() );
	...
	void TaskManager::resumeTask( Task& task )
	{
		...
		GC_ASSERT_RETURN( task.m_state == TS_PAUSED, "Tried to resume a non paused task! Task : " << task.name() );
	...
	{
		GC_ASSERT_RETURN( task.m_taskManager == this , "Tried to change the priority of a task not registered in this task manager!" );
		GC_ASSERT_RETURN( std::find( m_registeredTasksList.begin(), m_registeredTasksList.end(), &task) != m_registeredTasksList.end() , "Tried to activate task not registered in this TaskManager! Task : " << task.name() );
	...
	void TaskManager::executeTasks()
	{
		...
		*/
		...
		for( std::vector< Task* >::iterator taskCursor = m_executionTaskList.begin(); taskCursor != m_executionTaskList.end() ; ++taskCursor )
		{
			...
			{
				GC_ASSERT_OR( task->m_state == TS_ACTIVE, "Found an non active task in execution list! Task :" << task->name() ) continue;
				GC_ASSERT_OR( task->m_taskManager == this, "Found an active task in execution list that is not managed here! Task :" << task->name() ) continue;
				task->onExecute();
			}
	...
	void TaskManager::terminateAllTasks()
	{
		...
		for(std::list<Task*>::iterator itTask = tasksToTerminate.begin(); itTask != tasksToTerminate.end() ; ++itTask)
		{
			...
			GC_ASSERT_OR( task != nullptr , "Found a null task in the task manager!" ) continue;
			GC_ASSERT_OR( std::find( m_registeredTasksList.begin(), m_registeredTasksList.end(), task) != m_registeredTasksList.end() , "Tried to activate task not registered in this TaskManager! Task : " << task->name() ) continue;
			//change state
			task->m_state = TS_REGISTERED;
			//user defined termination
			task->onTerminate();
