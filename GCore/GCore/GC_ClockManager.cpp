...
namespace gcore
{
	...
	*/
	ClockManager::ClockManager(const TimeReference& timeReference, size_t reserveClockCount )
	...
	*/
	Clock* ClockManager::createClock(const std::string& name)
	{
	...
	*/
	void ClockManager::destroyClock(Clock* clock)
	{
		...
		GC_ASSERT( std::count( m_clockList.begin(), m_clockList.end(), clock ) == 1 , std::string( "Tried to destroy a clock that was not created by this manager! Clock name : ") + clock->name() );
	...
	*/
	Clock* ClockManager::getClock(const std::string& name)
	{
