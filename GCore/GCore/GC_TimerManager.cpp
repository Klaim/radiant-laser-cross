...
namespace gcore
{
	...
	Timer* TimerManager::createTimer( const Clock& clock, const std::string& name /*= "" */ )
	{
		GC_ASSERT( m_namedTimersIndex.find( name ) == m_namedTimersIndex.end() , std::string("Tried to create a timer with a name already used! Name : ") + name );
	...
	void TimerManager::destroyTimer( Timer* timer )
	{
		...
		GC_ASSERT( m_timerPool->is_from( timer ) , std::string( "Tried to destroy a timer that was not created by this manager! Timer name : ") + timer->getName() );
