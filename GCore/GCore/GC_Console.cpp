...
namespace gcore
{
	const std::string Console::DEFAULT_PREFIX( "/" );
	...
	void Console::parseParameters(const std::string& paramString, std::vector< std::string >& paramList,const std::string& paramSeparators)
	{
		paramList.clear();

		typedef boost::tokenizer< boost::char_separator<std::string::value_type>, std::string::const_iterator,	std::string > my_tok;

		boost::char_separator<std::string::value_type> sep( paramSeparators.c_str() );
	...
	long Console::injectKeys(const std::string& keys)
	{
		...
		if( overLength > 0 )// we don't take all the characters
		{
			...
			if( finalLength > 0 )
			{
				const std::string finalKeys = keys.substr( 0, finalLength );	
	...
	void Console::insertEntryText( const std::string& text )
	{
	...
	void Console::printText(const std::string& text)
	{
	...
	void Console::executeEntry()
	{
		...
		{
			const std::string commandName = extractCommandCall( m_entry );
			...
			if(!commandName.empty())
			{
				...
				if(it!=m_commandIndex.end())
				{
					...
					//get the parameters : 
					const std::string params = m_entry.substr( commandName.length() + m_commandCallPrefix.length() );
				...
				{
					//command not found!
					printText( "'" + commandName + "' command unknown.");
	...
	ConsoleCommandPtr Console::command(const std::string& name)
	{
	...
	void Console::defaultCommand(const std::string& name)
	{
	...
	void Console::commandCallPrefix( const std::string& commandCallPrefix )
	{
	...
	void Console::entry( const std::string& entry )
	{
		m_entry = entry;
	...
	std::string Console::extractCommandCall( const std::string& entry ) const
	{
		const std::string trimmedEntry = boost::algorithm::trim_copy( entry );
		if(trimmedEntry.empty()) return ""; // be lazy!
		...
		if( boost::algorithm::starts_with( trimmedEntry , m_commandCallPrefix ) ) 
		{
			...
			const std::string commandName = m_entry.substr( prefixLength, m_entry.find_first_of(" ") - prefixLength);
