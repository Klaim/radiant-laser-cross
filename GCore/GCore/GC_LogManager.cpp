...
namespace gcore
{
	...
	const std::string LogManager::DEFAULT_LOG( "lastSession.log" );

	LogManager::LogManager(const std::string& defaultLogName)
	...
	void LogManager::addLogListener( LogListener* logListener, const std::string& logName )
	{
		GC_ASSERT( logListener != nullptr, std::string( "Tried to add a null listener to log " ) + logName );
	...
	void LogManager::removeLogListener( LogListener* logListener, const std::string& logName )
	{
		GC_ASSERT( logListener != nullptr, std::string( "Tried to remove a null log listener from log " ) + logName );
	...
	//Create a new Log
	Log* LogManager::createLog(const std::string& name,bool isNewFile)
	{
	...
	void LogManager::destroyLog( const std::string& name )
	{
		...
		GC_ASSERT( it != m_logList.end(), std::string( "Tried to destroy a log not created in the log manager! Log name : ") + name );
		delete it->second;
		m_logList.erase( it );
	}
	...
	void LogManager::logMessage( const std::string& logName, const std::string& message )
	{
