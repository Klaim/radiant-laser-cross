...
namespace gcore
{
	...
	Log::Log( const LogManager& logManager, const std::string& name, bool isNewFile )
	...
	void Log::addText( const std::string& text )
	{
		m_message << text;
	}
	...
	void Log::logText()
	{
		...
		const std::string& finalMsg = finalMsgStream.str();
